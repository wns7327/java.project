			...
			if (b.isCollideWithBlock(this)) {
				for (int i = 0; i < BLOCK_ROWS; i++) { // 블럭 생성
					for (int j = 0; j < BLOCK_COLUMNS; j++) {
						...
						if (isHidden == false) {
							if (b.getRightCenter().x > x ) {           // 블럭의 왼쪽에 충돌
								b.x_direction *= -1;
								isHidden = true;
							...
							if (b.getBottomCenter().y > y + BLOCK_HEIGHT) {    // 블럭의 상단에 충돌
								b.y_direction *= -1;
								isHidden = true;
								rootComponent.score += (color + 1) * 10;
							}

							if (b.getLeftCenter().x < x + BLOCK_WIDTH) {    // 블럭의 우쪽에 충돌
								b.x_direction *= -1;
								isHidden = true;
								rootComponent.score += (color + 1) * 10;
							...
							if (b.getTopCenter().y < y) {			// 블럭의 하단에 충돌
								b.y_direction *= -1;
								isHidden = true;
								rootComponent.score += (color + 1) * 10;
							} 
